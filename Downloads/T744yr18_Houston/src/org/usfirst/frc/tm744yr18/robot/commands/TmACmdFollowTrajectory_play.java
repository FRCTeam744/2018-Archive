 ...
 *
 */
public class TmACmdFollowTrajectory_play extends Command {
	 ...
	 * 
	 */
    public TmACmdFollowTrajectory_play(String filename, char switchSide, char scaleSide) {
    ...
    public TmACmdFollowTrajectory_play(String filename, char switchSide, char scaleSide, boolean showFile) {
...
//    				- tWorker.trajDataCsvL.get(traj_index_left).getLeftPos();
    		...
    		if(Tm744Opts.isUseNoGyros()) {
    			...
    			if(Tm744Opts.isGyroNavX()) {
    				thetaError = tWorker.trajDataCsvL.get(traj_index_left).getAngle() - TmSsDriveTrain.DrvNavX.getGlobalAngle(); //done//COMMENT THIS IN WHEN NAVX IS READY! AND COMMENT OUT PREVIOUS LINE
    			} else {
    				thetaError = tWorker.trajDataCsvL.get(traj_index_left).getAngle() - TmSsDriveTrain.DrvGyro.getAngle();
    			}
    		}
...
//    				leftError);
    		if(Tm744Opts.isGyroNavX()) {
    			TmSdMgr.putNumber(SdKeysE.KEY_NAVX_IMU_TOTAL_YAW, TmSsDriveTrain.DrvNavX.getGlobalAngle()); //done//COMMENT THIS IN WHEN NAVX IS READY! AND COMMENT OUT PREVIOUS LINE
    		} else {
    			TmSdMgr.putNumber(SdKeysE.KEY_DRIVE_GYRO_ANGLE, TmSsDriveTrain.DrvGyro.getAngle());
    		}
