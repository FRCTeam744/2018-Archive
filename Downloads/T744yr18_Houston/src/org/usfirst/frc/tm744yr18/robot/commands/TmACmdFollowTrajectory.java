...
//    				- tWorker.trajDataCsvL.get(traj_index_left).getLeftPos();
    		...
    		if(Tm744Opts.isUseNoGyros()) {
    			...
    			if(Tm744Opts.isGyroNavX()) {
    				thetaError = tWorker.trajDataCsvL.get(traj_index_left).getAngle() - DrvNavX.getLocalAngle(); //done //COMMENT THIS IN WHEN NAVX IS READY! AND COMMENT OUT PREVIOUS LINE
    			} else {
    	...
    	//	double thetaError = tWorker.trajDataCsvL.get(traj_index_left).getAngle() - ;
